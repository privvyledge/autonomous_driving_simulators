...
```
dependency failed to start: container carla-server is unhealthy
```

- Check GPU driver and VRAM: `nvidia-smi`
  - Look at the `Processes:` table. Other CUDA compute processes (ML workloads, previous container runs) can saturate the GPU and prevent CARLA from initialising. If GPU utilisation is near 100% or VRAM is largely consumed, stop those processes and retry.
  - CARLA at `Low` quality needs roughly 2–4 GB of free VRAM on startup.
- Verify NVIDIA runtime: `docker info | grep -i runtime`
- Port conflict: `ss -tlnp | grep 2000`
- Check the full CARLA log for the actual crash message: `docker compose logs --tail=300 carla-server`
...
**Show the CARLA Unreal Engine window**

By default CARLA runs offscreen. To open the UE4 window on your host display:

```bash
# Grant X11 access to Docker containers
xhost +local:docker

# Start the stack — the carla-server container auto-detects the X socket
# and omits -RenderOffScreen when a display is available
DISPLAY=:0 docker compose up
```

To make this permanent without editing tracked files, copy the override example and uncomment the GUI section:

```bash
cp docker-compose.override.yml.example docker-compose.override.yml
# The carla-server block in the example sets SDL_VIDEODRIVER=x11 and
# removes -RenderOffScreen. Edit quality level there too if desired.
docker compose up
```

Note: rendering to a visible window consumes more VRAM and GPU compute than offscreen mode.
